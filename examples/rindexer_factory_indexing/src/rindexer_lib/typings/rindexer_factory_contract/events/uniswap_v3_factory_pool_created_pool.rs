    ...
    TExtensions: Send + Sync + 'static,
{
    ...
    where
        ...
        Fut: Future<Output = EventCallbackResult<()>> + Send + 'static,
    {
        let csv_path = std::env::current_dir().expect("Failed to get current directory").join(r"generated_csv/UniswapV3FactoryPoolCreatedPool/uniswapv3factorypoolcreatedpool-poolcreated.csv");
